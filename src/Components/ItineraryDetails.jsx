      ...
      {!steps || steps.length === 0 ? (
        ...
        <div className="steps-container mt-3">
          {steps.map((step, index) => {
            ...
            const line = isTransit ? step.transit.line : null;
            const mode = line?.vehicle?.type || step.travel_mode || transport;

            // Infos ligne de transport
            const lineName = line?.short_name || line?.name || line?.agencies || "Ligne inconnue";
            const departureTime = step.transit?.departure_time?.text || step.transit?.departure || "Non précisé";
            const arrivalTime = step.transit?.arrival_time?.text || step.transit?.arrival || "Non précisé";
              ...
              <div key={index} className="step-item mb-3 p-3 bg-white rounded shadow-sm">
                <div className="d-flex align-items-start">
                  ...
                  <div className="step-content flex-grow-1">
                    <div className="fw-bold">{stripHtml(step.html_instructions || step.instruction)}</div>

                    {(step.distance || step.duration) && (
                      <div className="step-meta text-muted small mt-2">
                        {step.distance && (
                          <span className="me-3">
                            <i className="bi bi-signpost me-1"></i>{step.distance}
                          </span>
                        )}
                        {step.duration && (
                          <span>
                            <i className="bi bi-clock me-1"></i>{step.duration}
                          </span>
                        )}
                      </div>
                    )}
                      ...
                      <div className="transit-details bg-light p-2 mt-2 small rounded border">
                        <div className="d-flex flex-wrap">
                          <div className="me-3 mb-1">
                            <strong>Ligne :</strong> {lineName}
                          </div>
                          <div className="me-3 mb-1">
                            <strong>Départ :</strong> {departureTime}
                          </div>
                          <div className="me-3 mb-1">
                            <strong>Arrivée :</strong> {arrivalTime}
                          </div>
                          {typeof numStops === 'number' && numStops > 0 && (
                            <div className="mb-1">
                              <strong>Arrêts :</strong> {numStops}
                            </div>
                          )}
                        </div>
                      </div>
